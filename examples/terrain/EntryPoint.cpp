...
Mesh CreatePlaneMesh(std::vector<MaterialData>& globalMaterials, std::vector<Image>& globalImages)
{
    ...
    for (unsigned i = 0; i <= resolution - 1; i++)
    {
        ...
        {
            static Vector3 normal = { 0.0f, 0.0f, 1.0f };

            float x = (-width / 2.0f + width * i / (float)resolution);
            float y = (0.0f); 
            float z = (-height / 2.0f + height * j / (float)resolution); 
            float u = (i / (float)resolution); 
            float v = (j / (float)resolution); 
            auto v0 = ModelData::Vertex{ { x,z,y }, {u,v }, normal };

            x = (-width / 2.0f + width * (i + 1) / (float)resolution); 
            y = (0.0f); 
            z = (-height / 2.0f + height * j / (float)resolution); 
            u = ((i + 1) / (float)resolution); 
            v = (j / (float)resolution); 
            auto v1 = ModelData::Vertex{ { x,z,y }, {u,v }, normal };

            x = (-width / 2.0f + width * i / (float)resolution); 
            y = (0.0f); 
            z = (-height / 2.0f + height * (j + 1) / (float)resolution); 
            u = (i / (float)resolution); 
            v = ((j + 1) / (float)resolution); 
            auto v2 = ModelData::Vertex{ { x,z,y }, {u,v }, normal };
            ...
            u = ((i + 1) / (float)resolution); 
            v = ((j + 1) / (float)resolution);
            auto v3 = ModelData::Vertex{ { x,z,y }, {u,v }, normal };
    ...
    std::array textures = {
        ImageData{ std::vector<uint8_t>{ 150, 225, 100, 255 }, Format::R8G8B8A8_UNORM, 1, 1 },
        ImageData{std::vector < uint8_t>{ 127, 127, 255, 255 }, Format::R8G8B8A8_UNORM, 1, 1 }, };

    return CreateMesh(vertices, indices, instances, MakeView(textures), globalImages);
}
...
public:
    ...
    virtual void SetupPipeline(PipelineState pipeline) override
    {
        auto vertexShader = ShaderLoader::LoadFromSourceFile("main_vertex.glsl", ShaderType::VERTEX, ShaderLanguage::GLSL);
        auto tessControlShader = ShaderLoader::LoadFromSourceFile("tess_control.tesc", ShaderType::TESS_CONTROL, ShaderLanguage::GLSL);
        auto tessEvalShader = ShaderLoader::LoadFromSourceFile("tess_eval.tese", ShaderType::TESS_EVALUATION, ShaderLanguage::GLSL);
        auto fragmentShader = ShaderLoader::LoadFromSourceFile("main_fragment.glsl", ShaderType::FRAGMENT, ShaderLanguage::GLSL);
        pipeline.Shader = std::make_unique<GraphicShader>(vertexShader, tessControlShader, tessEvalShader, fragmentShader);

        pipeline.SetFillMode(FillMode::FRAME_WIRE);
...
int main()
{
    ...
    while (!window.ShouldClose())
    {
        ...
        if (Vulkan.IsRenderingEnabled())
        {
            ...
            uniformSubmitPass.CameraUniform.ViewProjectionMatrix = camera.GetMatrix();
            uniformSubmitPass.CameraUniform.ViewMatrix = camera.GetViewMatrix();
